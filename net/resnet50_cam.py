...
class Net_CLIP(Net):
    def __init__(self, stride=16, n_classes=20, text_dim=512):
        super(Net_CLIP, self).__init__(stride, n_classes)
        self.text_dim = text_dim
        self.n_classes = n_classes
        self.conv1 = nn.Conv2d(2048, self.text_dim, 1, bias=False)
        self.classifier = nn.Conv2d(self.text_dim, n_classes, 1, bias=False)
        self.newly_added = nn.ModuleList([self.conv1, self.classifier])

    def initialize_clip_classifier(self, file_path, frozen=True):
        # file shape: [n_classes, text_dim]
        weights = torch.load(file_path, map_location='cuda')    # [n_classes, text_dim]
        self.classifier.weight.data = weights[:, :, None, None]
        if frozen:
            self.classifier.weight.requires_grad = False
            print('Classifier is frozen.')
        print('Classifier is initialized by CLIP text embedding from {}.'.format(file_path))

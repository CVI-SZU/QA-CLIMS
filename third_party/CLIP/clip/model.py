...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.mask_pool = nn.AvgPool2d(patch_size, stride=patch_size)    # ks=14, stride=14
        self.mask_prompt_depth = mask_prompt_depth
        self.mask_embedding = nn.Parameter(torch.zeros(self.mask_prompt_depth, self.grid_size * self.grid_size, width)) # 3, 256, 1024

    def forward(self, x: torch.Tensor, m: torch.Tensor = None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]  # bs, 1024, 16, 16
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]  # bs, 1024, 256
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] # bs, 256, 1024
        if m is not None:
            assert len(m.shape) == 4, "Mask should be 4D (bs, 1, h, w)"
            m = self.mask_pool(m.to(torch.float).squeeze(1)).reshape(m.shape[0], -1).unsqueeze(-1)   # bs, 256, 1
            # m = self.mask_pool(m.to(torch.float).squeeze()).reshape(m.shape[0], -1).unsqueeze(-1)   # bs, 256, 1
            m = torch.ceil(m)   # bs, 256, 1
